...
Rust Types ──>│   hub-macro     │──JSON──>│   IR Builder    │──JSON──>│   Generators    │──> TypeScript
              ...
              {backend}.schema           emits IR JSON               rpc.ts, namespaces/*
```
...
// Output: JSON Schema (served via {backend}.schema RPC)
{
  "method": "chat",
...
Synapse walks the schema tree via `{backend}.schema` RPC, building a deduplicated IR:
