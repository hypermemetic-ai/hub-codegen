# Unify {backend}.call with Streaming Pattern

## Problem

Currently `{backend}.call` is a special case:
...
Make `{backend}.call` return a stream like everything else:
...
1. Any code using `{backend}.call` needs to handle `CallEvent` instead of raw `PlexusStreamItem`
